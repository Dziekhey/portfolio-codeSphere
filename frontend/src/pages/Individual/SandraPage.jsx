...
const SandraPage = () => {
  ...
  const [sandfirstskills, setSandfirstskills] = useState([]);
  const [sandsecondskills, setSandsecondskills] = useState([]);
  const [comexperience, setComexperience] = useState([]);

  const fetchData = async () => {
    try {
      const firstskills = await apiGetUserSkills("Sandra");
      // console.log(skills.data);
      const firstdata = firstskills.data.filter(
        (res) => res?.type === "Programming Languages and Libraries"
      );

      const secondskills = await apiGetUserSkills("Sandra");
      ...
      setSandfirstskills(firstdata);
      setSandsecondskills(seconddata);
          ...
          <div className="w-full mx-24 flex lgl:flex-row gap-10 lgl:gap-20">
            <div className="w-1/2 lgl:w-1/2">
              ...
              <div className="w-full lgl:w-1/2">
                {sandfirstskills.length > 0 &&
                  sandfirstskills.map((data) => (
            ...
            <div className="w-1/2 lgl:w-1/2">
              ...
              <div className="w-full lgl:w-1/2">
                {sandsecondskills.length > 0 &&
                  sandsecondskills.map((data) => (
                    <SkillCard key={data._id} skills={data} />
                  ))}
              </div>
            </div>
        ...
        <div className="p-8 border-t-4 border-black">
          ...
          <div className="flex justify-center space-x-10 font-medium py-4">
            <span size="small" className="hover:scale-105 cursor-pointer">
              <ion-icon name="mail-outline"></ion-icon>dakeysandra@gmail.com
            </span>
            <span size="small" className="hover:scale-105 cursor-pointer">
              <ion-icon name="call-outline"></ion-icon>+233504062107
            </span>
            <span size="small" className="hover:scale-105 cursor-pointer">
